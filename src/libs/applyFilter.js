export default function applyFilter(canvasRef, src, rangeValue, filter){
    ...
    if(filter == "Brightness"){
      ...
      var sv = rangeValue / 20; // saturation value. 0 = grayscale, 1 = original
...
// Put the modified image data back onto the canvas
        ...
        // Function to blend the rectangle
        
            // Create a semi-transparent rectangle
            ctx.globalAlpha = rangeValue / 100; // Adjust the opacity as needed
            ctx.fillStyle = "blue"; // Color of the rectangle
            ctx.fillRect(0, 0, canvas.width, canvas.height);

            // Reset the global alpha to 1 (fully opaque)
            ctx.globalAlpha = 1;
    }
